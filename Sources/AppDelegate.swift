...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    /// 注意：参数传入 isDark，禁止在内部访问 ThemeManager.shared ——
    /// ThemeManager.init() 会调 applyMode() 走到这里，若再取 .shared 会在
    /// dispatch_once 未完成时递归进入自身，直接 SIGTRAP（libdispatch recursive lock）。
    static func refreshGlobalAppearance(dark: Bool) {
    ...
    private func applyGlobalNightModeAppearance() {
        AppDelegate.refreshGlobalAppearance(dark: ThemeManager.shared.isDark)
    }
